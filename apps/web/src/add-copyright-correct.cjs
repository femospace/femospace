...
while ((match = langHeadRegex.exec(resourcesText)) !== null) {
    ...
    if (endIndex !== -1) {
        langBlocks.push({
            code: langCode,
            fullMatch: resourcesText.substring(match.index, endIndex + 1),
...
for (const block of langBlocks) {
    ...
    try {
        ...
        // CORRECT ORDER: masterEn as base, langData as source of truth for translations
        const merged = mergeDeep(masterEn, langData);
        
        const newBlockText = `'${block.code}': {
    translation: ${safeStringify(merged)}
  }`;
        
        updatedResourcesText = updatedResourcesText.replace(block.fullMatch.trim().replace(/,$/, ''), newBlockText);
    } catch (e) {
...
// Ensure missing commas are added between languages
content = content.replace(/}\s*'\s*([a-zA-Z-]+)':/g, '},\n  \'$1\':');

fs.writeFileSync(i18nPath, content);
console.log('Successfully added copyright and rebuilt all languages with proper merging.');
