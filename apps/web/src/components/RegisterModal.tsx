...
export const RegisterModal = ({ isOpen, onClose, onSwitchToLogin }: RegisterModalProps) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const lastName = nameParts.slice(1).join(' ') || 'User'; // Fallback last name
    ...
    <AnimatePresence>
      {isOpen && (
        <>
          ...
          >
            <motion.div
              className="bg-white dark:bg-gray-800 rounded-2xl shadow-2xl w-full max-w-md p-8 relative my-8"
            ...
            >
              ...
              <div className="mb-8">
                ...
                <p className="text-gray-600 dark:text-gray-400">
                  {t('auth.createAccount')} {t('welcome.title')}
                </p>
              </div>
              ...
              <form onSubmit={handleSubmit} className="space-y-5">
                ...
                <motion.button
                  type="submit"
                  disabled={isLoading}
                  className="w-full bg-gradient-to-r from-blue-500 to-blue-600 text-white font-semibold py-3 rounded-lg shadow-lg hover:shadow-xl transition-all flex items-center justify-center gap-2 disabled:opacity-50 disabled:cursor-not-allowed mt-6"
